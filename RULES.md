...
| TMP0143 | Error | Workflow code uses raw task scheduling | Raw Task combinators (WhenAll/WhenAny/ContinueWith) schedule continuations on the default TaskScheduler rather than the deterministic workflow scheduler. Prefer Workflow.WhenAllAsync / Workflow.WhenAnyAsync, and use .Cancel() instead of CancellationTokenSource.CancelAsync(). |
...
| TMP3205 | Warning | Invalid workflow signal method | A signal handler must return Task; returning void, Task<T>, or a value is not allowed. |
