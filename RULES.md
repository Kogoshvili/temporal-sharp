...
| TMP2132 | Warning | Base Exception thrown instead of ApplicationFailure | A non-ApplicationFailure exception from a workflow silently retries the task forever. Throw Temporalio.Exceptions.ApplicationFailure with a typed message. |
...
| TMP3208 | Warning | Invalid workflow update return type | An update handler must return a concrete Task<T>; returning void, a plain value, or a non-generic Task makes the update result unusable by callers. |
